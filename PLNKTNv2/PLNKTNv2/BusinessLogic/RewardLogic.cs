...
namespace PLNKTNv2.BusinessLogic
{
    ...
    {
        public void CalculateUserRewardCompletion()
        {
            throw new NotImplementedException();
        }

        private User CalculateUserRewardCompletion(User user, IMessenger helper)
        {
            /* Algorithm
            ...
            foreach (var _reward in user.UserRewards.Where(ur => ur.Status != UserRewardStatus.Complete))
            {
                ...
                if (incompleteChallenges == 0)
                {
                    ...
                    changesMade = true;
                    helper.AddLine(user.Id, "Reward", _reward.Id);
                }
            }
        ...
        */
        private User CalculateUserChallengeCompletion(User user, IMessenger email)
        {
            /* Algorithm
            ...
            //IEmailHelper email = new EmailLogic();
            ...
            foreach (var _reward in user.UserRewards.Where(ur => ur.Status != UserRewardStatus.Complete))
            {
                ...
                foreach (var _challenge in _reward.Challenges.Where(c => c.Status != UserRewardChallengeStatus.Complete))
                {
                    ...
                    if (isSuccessful)
                    {
                        ...
                        changesMade = true;
                        email.AddLine(user.Id, "Challenge", _challenge.Id);
                    }
                }
            }
